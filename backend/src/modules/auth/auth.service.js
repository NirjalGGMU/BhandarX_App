...
class AuthService {
  ...
  async forgotPassword(emailData) {
    ...
    // Generate reset token for web flow
    const { resetToken, hashedToken } = TokenHelper.generateResetToken();
    // Generate OTP for mobile flow
    const otpCode = this._generateOtpCode();
    const hashedOtp = TokenHelper.hashToken(otpCode);

    // Set token expiry (15 minutes)
    const tokenExpiry = Date.now() + 15 * 60 * 1000;

    // Save hashed token + OTP to database
    await authRepository.setResetPasswordToken(user._id, hashedToken, tokenExpiry);
    await authRepository.setResetPasswordOtp(user._id, hashedOtp, tokenExpiry);

    // Send email
    const isDev = process.env.NODE_ENV !== 'production';
    try {
      const emailService = require('../../shared/services/email.service');
      // Keep web link flow + mobile OTP in one email cycle.
      if (!isDev) {
        await emailService.sendPasswordResetEmail(user, resetToken);
      }
      await emailService.sendPasswordResetOtpEmail(user, otpCode);
    } catch (error) {
      const logger = require('../../config/logger');
      logger.error(`Failed to send reset email/otp: ${error.message}`);
      if (!isDev) {
        throw error;
      }
    }
    ...
    const devOtp = isDev ? otpCode : undefined;
  ...
  async resetPasswordWithOtp(resetData) {
    const resetPasswordDTO = new ResetPasswordDTO(resetData);
    const otpHash = TokenHelper.hashToken(resetPasswordDTO.otp);
    const user = await authRepository.findUserByResetOtp(
